        ...
        <div class="bg-white shadow-md rounded my-6 p-5">
          <form method="POST" action="{{ route('admin.users.store')}}">
            ...
            <div class="flex flex-col space-y-2">
              ...
              <input id="phone" type="tel" name="phone" value="{{ old('phone') }}" pattern="[0-9]{10}" title="Please enter a 10-digit phone number without any spaces or special characters. Example: 0123456789" placeholder="Enter phone" class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200" />
            </div>
              ...
              <div class="flex flex-col justify-cente">
                <div class="flex flex-col">
                  <label class="inline-flex items-center mt-3">
                    <input type="radio" class="form-radio h-5 w-5 text-blue-600" name="roles[]" value="{{$role->id}}" title="You have to choose a role for user" required><span class="ml-2 text-gray-700">{{ $role->name }}</span>
                  </label>
                </div>
              </div>
              @endforeach
