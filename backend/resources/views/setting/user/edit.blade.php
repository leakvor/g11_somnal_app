                    ...
                    <form id="userForm" method="POST" action="{{ route('admin.users.update', $user->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mt-3 relative">
                            <label for="user-image" class="d-block">
                                <img src="{{ $user->profile ? asset('/'.$user->profile) : asset('images/default-profile.png') }}"
                            ...
                            <button type="button" id="show-button"
                                class="btn btn-primary  absolute top-0 left-44 " onclick="clearFileInput()">
                        ...
                        <div class="flex flex-col space-y-2">
                            ...
                            <small id="nameError" class="text-red-600 hidden">User Name must start with a capital letter
                                and contain
                                only letters</small>
                        </div>
                        ...
                        <div class="flex flex-col space-y-2 mt-3">
                            ...
                            <small id="phoneError" class="text-red-600 hidden">Please enter a valid phone number
                                starting with 0 and
                        ...
                        <div class="flex flex-col space-y-2 mt-3">
                            ...
                            <input id="password" type="password" name="password" placeholder="Enter password"
                                class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200"
                                minlength="8">
                            <small id="passwordError" class="text-red-600 hidden">Password must be at least 8 characters
                                long</small>
                        </div>
                        ...
                        <div class="flex flex-col space-y-2 mt-3">
                            ...
                            <input id="password_confirmation" type="password" name="password_confirmation"
                                ...
                                class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200"
                                minlength="8">
                            <small id="passwordConfirmationError" class="text-red-600 hidden">Passwords do not
                                match</small>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const defaultImageURL = "{{asset('images/default-profile.png')}}";
    document.getElementById('preview-image').src = defaultImageURL; // Reset the preview image to the default

    // Make an AJAX request to clear the profile image from the database
    fetch('{{ route("admin.users.update", $user->id) }}', {
        ...
        if (response.ok) {
            ...
            console.log('Failed to clear profile image.');
        }
    });
}
