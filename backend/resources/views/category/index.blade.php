    ...
    <div id="search-container" class=" flex justify-end pr-5">
        <div class="relative pr-4 lg:mx-0">
          ...
          <input id="searchInput" class="form-input w-full md:w-64 rounded-md pl-10 pr-4 focus:border-indigo-600"
            type="text" placeholder="Search Category..." onkeyup="searchUsers(this.value)">
          ...
          <tbody>
            @can('category access')
              @foreach ($categories as $category)
                ...
                <td class="py-4 px-6 border-b border-grey-light" id="name">{{ $category->name }}</td>
                <td class="py-4 px-6 border-b border-grey-light text-right">
                    @can('category edit')
                      <a href="{{route('admin.categories.edit',$category->id)}}" class="px-4 py-2 font-medium text-white bg-green-700 rounded-md hover:bg-blue-500 focus:outline-none focus:shadow-outline-blue active:bg-blue-600 transition duration-150 ease-in-out">Edit</a>
                    @endcan
